...
The system SHALL support loading user-defined configuration from an external INI configuration file located in the same directory as the main script.

#### Scenario: Config file exists with valid settings
- **WHEN** a `health_check.cfg` file exists in the script directory with valid configuration
...
- **WHEN** no `health_check.cfg` file exists in the script directory
...
- **WHEN** a `health_check.cfg` file exists but contains INI syntax errors
...
- **WHEN** a `health_check.cfg` file exists with only some configuration values defined
...
- **WHEN** threshold values (e.g., filesystem_warning) are outside the range 0-100
...
#### Scenario: Unknown configuration sections or keys
- **WHEN** the config file contains sections or keys not recognized by the script
...
- **THEN** a `health_check.cfg.example` file SHALL be present
...
- **WHEN** user copies `health_check.cfg.example` to `health_check.cfg`
...
- **WHEN** a user creates `health_check.cfg` in their working directory
...
- **WHEN** the repository contains `health_check.cfg.example`
...
- **WHEN** `health_check.cfg` exists with mode 0644 or more permissive
...
- **WHEN** `health_check.cfg` exists with mode 0600
...
- **WHEN** both `linux_health_check.py` and `health_check.cfg` are in the same directory
